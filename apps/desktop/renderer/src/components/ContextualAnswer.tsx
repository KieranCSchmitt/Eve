...
export function ContextualAnswer({ response, message, pending, onDismiss, onOpenSource }: {
  ...
  const text = message || response?.message || "Eve finished without a response.";
  ...
  if (pending || (!response && !message)) return null;
  return <aside className="contextual-answer" aria-label="About this passage">
    <div className="contextual-answer-heading"><span>{response?.basis === "general" ? "General knowledge" : response?.citations.length ? "From your sources" : "Eve"}</span>
      <button className="icon-button" aria-label="Dismiss explanation" title="Dismiss explanation" onClick={onDismiss}><X size={15} /></button></div>
